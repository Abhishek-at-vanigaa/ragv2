...
const ChatComponent: React.FC = () => {
  ...
  const [selectedModel, setselectedModel] = useState<string>(""); // State to store selected value
  ...
  const handleSubmit = async (e: any) => {
    ...
    setLoading(true);
    const response = await getBotResponse(input, bearer,selectedModel);
  ...
  const handleFileUpload = async (files: FileList | null) => {
    ...
    if (files) {
      ...
      // console.log("Files ready to be uploaded: ", selectedFiles, formData);
  ...
  const handleSelectChange = (event: React.ChangeEvent<HTMLSelectElement>) => {
    setselectedModel(event.target.value); // Update selected value when the select box value changes
  };
  useEffect(() => {
    WebViewer(
      {
        ...
        <div className="flex border-r border-solid border-neutral-border pt-6 pr-6 pb-6 pl-6 h-full flex-col gap-4 items-start">
          <div className="flex-none h-10 w-full  ">
            ...
            <select
              ...
              className="flex-none h-10 w-full rounded-md text-black"
            >
              <option className=" " value="">
                Select model ...
              </option>
              <option className="" value="openAi">
                Open AI
              </option>
              <option className="" value="gpt4All">
                Gpt - 4 - All
              </option>
            </select>
            {/* {selectedModel && <p>Selected Value: {selectedModel}</p>} */}
          </div>
          <input
            type="file"
            ref={fileInputRef}
